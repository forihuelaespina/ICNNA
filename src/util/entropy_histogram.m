function [result,descriptor]=entropy_histogram(x,descriptor)
%HISTOGRAM Computes the frequency histogram of the row vector x.
%   [RESULT,DESCRIPTOR] = ENTROPY_HISTOGRAM(X) or
%   [RESULT,DESCRIPTOR] = ENTROPY_HISTOGRAM(X,DESCRIPTOR) or
%where
...
%
% 6-May-2019: FOE:
%   Function name collided with matlab latest histogram function, so
% I changed to entropy_histogram as this function was being used by
% the function entropy. I have also changed the calls in function
% entropy.
%


if nargin <1
   disp('Usage: RESULT = ENTROPY_HISTOGRAM(X)')
   disp('       RESULT = ENTROPY_HISTOGRAM(X,DESCRIPTOR)')
