function A=icnna_normalize(A,varargin)
%Normalizes data matrix
%
% A=icnna_normalize(A) the matrix is normalized to zero mean and unit variance
%   across all dimensions at once.
%
% A=icnna_normalize(A,dim) the matrix is subdivided in hyperplanes along dim x,
%   and each submatrix is normalized to zero mean and unit variance
%   separately.
%
% A= icnna_normalize(...,'normal',[mu var]) the matrix is normalized to 
%   have mu mean and var variance.
%
% A= icnna_normalize(...,'range',[min max]) the matrix is normalized to range
...
%
% 31-Aug-2025: FOE
%   + File created as a copy of now deprecated 'ICNNA/util/normalize'
%   
