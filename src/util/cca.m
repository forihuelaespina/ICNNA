...
k=0; fprintf(1, 'iterating: %d / %d epochs\r',k,epochs);
...
for i=1:train_len, 
  ...
  if rem(i,noc)==0, 
    k=k+1;
    if (mod(k,50)==0) %reduce verbosity
        fprintf(1, [datestr(now,13) ': iterating: %d / %d epochs\r'],k,epochs);
    end
  end
...
% calculate error
tmperror = cca_error(P,Mdist,lambda(train_len));
fprintf(1,'%d iterations, error %f          \n', epochs, tmperror);
