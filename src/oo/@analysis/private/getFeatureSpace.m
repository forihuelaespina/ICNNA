...
if ~isempty(get(obj,'SubjectsIncluded'))
    tmpStruct.dimension=obj.F.DIM_SUBJECT;
    tmpStruct.values=get(obj,'SubjectsIncluded');
    subsetDefinition(end+1)={tmpStruct};
end
if ~isempty(get(obj,'SessionsIncluded'))
    tmpStruct.dimension=obj.F.DIM_SESSION;
    tmpStruct.values=get(obj,'SessionsIncluded');
    subsetDefinition(end+1)={tmpStruct};
end
