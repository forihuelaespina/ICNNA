...
%% Remarks
%
% This is a loose analogous of the experimentSpace series SCHNBA
...
%           included
% 
%   .whichSamplingSites - Int[]. By default
%           all sampling sites are included.
%           The list of |id|s for the sampling sites to be included.
%           If empty i.e. .whichSamplingSites = [], then all elements are
%           included
...
%               Cell arrays of values is NOT supported.
%
%   .whichSamplingSiteNames - (String|char)[]. By default
%           all sampling sites are included.
%           The list of |name|s for the sampling sites to be included.
%           If empty i.e. .whichSamplingSiteNames = [], then all elements
%           are included
%               Cell arrays of values is NOT supported.
%
...
%      .topo - which will be set to false here
% 
...
if(exist('options','var'))
    ...
    if(isfield(options,'whichSamplingSiteNames'))
        opt.whichSamplingSiteNames = options.whichSamplingSiteNames;
    end
end
...
if ~isempty(opt.whichSamplingSites)
    c(end+1).column = 'SamplingSite.id';
    c(end).logic  = 'ismember';
    c(end).value  = opt.whichSamplingSite;
end
...
if ~isempty(opt.whichSamplingSiteNames)
    c(end+1).column = 'SamplingSite.name';
    c(end).logic  = 'ismember';
    c(end).value  = opt.whichSamplingSiteNames;
end
...
for iCase = 15232:nCases%1:nCases
    ...
    theSamplingSiteID   = tmpBcases{iCase,'SamplingSite.id'};
    ...
    theSamplingSiteName = tmpBcases{iCase,'SamplingSite.name'};
    ...
    disp(strcat(num2str(iCase),'/',num2str(nCases),': ',...
              ...
              '; site',  num2str(theSamplingSiteID,'%04i'), '-', theSamplingSiteName));
    ...
    c(5).column  = 'SamplingSite.id';
    c(5).logic   = 'ismember';
    c(5).value   = theSamplingSiteID;
    
    c(6).column  = 'ExperimentalUnit.name';
    c(6).logic   = 'ismember';
    c(6).value   = theUnitName;
    c(7).column  = 'Group.name';
    c(7).logic   = 'ismember';
    c(7).value   = theGroupName;
    c(8).column  = 'Session.name';
    c(8).logic   = 'ismember';
    c(8).value   = theSessionName;
    c(9).column  = 'Condition.name';
    c(9).logic   = 'ismember';
    c(9).value   = theConditionName;
    c(10).column = 'SamplingSite.name';
    c(10).logic  = 'ismember';
    c(10).value  = theSamplingSiteName;
    
    tmpS2 = icnna.op.getSubbundle(tmpS,'base',c);

    %Plot
    opt2 = opt;
    opt2.singleAxis         = true;
    opt2.topo               = false;
    opt2.whichSamplingSites = zeros(nSamplingSites,1);
    opt2.whichSamplingSites(theSamplingSiteID) = 1;
    [hFig] = icnna.plot.plotBundle_FamilyOfSpaces(tmpS2,opt2);
    if opt.save
        tmpTheSamplingSiteName = strrep(theSamplingSiteName,':','-');
        mySaveFig(hFig,strcat(opt.destinationFolder, ...
              'SingleSite_NonAveraged', ...
              ...
              '_site',  num2str(theSamplingSiteID,'%04i'), '-', tmpTheSamplingSiteName));
    end
    close(gcf);
