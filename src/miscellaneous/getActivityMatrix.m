...
%% Parameter
%
% db - a database as obtained from ICNA. For columns see
    ...
    for ch=1:nChannels
        ...
        if ~isempty(idx)
            ...
            switch (internalOpt_Test)
                ...
                case 'ttest' %Paired. Null= x-y comes from normal with zero mean
                    [~,oxy_pValue] = ttest(tmpDB(:,dbCons.COL_MEAN_TASK),...
                        tmpDB(:,dbCons.COL_MEAN_BASELINE),...
                        'alpha',opt.significanceLevel,'tail','both');
                case 'ttest2' %Non paired
            ...
            switch (internalOpt_Test)
                case 'SignRank'
                    deoxy_pValue = signrank(tmpDB(:,dbCons.COL_MEAN_TASK),...
                        ...
                        'alpha',opt.significanceLevel);
                case 'ttest' %Paired. Null= x-y comes from normal with zero mean
                    [~,deoxy_pValue] = ttest(tmpDB(:,dbCons.COL_MEAN_TASK),...
                        tmpDB(:,dbCons.COL_MEAN_BASELINE),...
                        'alpha',opt.significanceLevel,'tail','both');
                case 'ttest2'
    ...
    switch (opt.type)
        ...
        case 'oxy'
            M=[M; getPatterns_Oxy(chActivity_Sign,...
                chActivity_pValue,opt.significanceLevel)];
        otherwise
            error('ICNA:getActivityMatrix:InvalidType',...
...
%and add one colour for pattern 17 or 5 (missing data)
cmap=[cmap; 0 0 0];
...
hMain=axes();
set(gca,'OuterPosition',[0 0.25 1 0.75]);
...
hLegend=axes();
set(gca,'OuterPosition',[0 0 1 0.25]);
